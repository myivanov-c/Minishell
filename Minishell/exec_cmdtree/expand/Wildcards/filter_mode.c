...
void	filter_mode_loops(char *str, int *start, int *end, int *word)
{
	int	i;
...
int	filter_mode(char *str)
{
	...
	if (has_start && has_word && !has_end)
		return (1);
	else if (!has_start && has_word && has_end)
		return (2);
	else if (!has_start && has_word && !has_end)
		return (3);
	else if (has_start && has_word && has_end)
...
int	filter_mode_helper(char *line_to_compare, t_wildcard *w)
{
	...
	{
		if (ft_strncmp(line_to_compare + i, w->split_chars[j],
				ft_strlen(w->split_chars[j])) == 0)
		{
